...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    {
        MessageBox(NULL, "Error while starting the program", "Activate Windows 10/11", MB_ICONEXCLAMATION);
        PostQuitMessage(0);
    	return 0;
    }
    
    if(!Is_Win10_or_Later()){
    	MessageBox(NULL, "You must have Windows 10 or Windows 11 to run the program", "Activate Windows 10/11", MB_ICONERROR);
		PostQuitMessage(0);
    	return 0;
	}
   
    // TWORZENIE OKNA
    hwnd = CreateWindowEx(0, NazwaKlasy, "Activate Windows 10/11", WS_SYSMENU, 0, 0, 320, 245, NULL, NULL, hInstance, NULL);
   ...
   HANDLE hMutex = CreateMutex(NULL, FALSE, "AW10/11");
	if(hMutex && GetLastError() != 0){
    	MessageBox(NULL, "This application is already running", "Activate Windows 10/11", MB_ICONEXCLAMATION);
    	PostQuitMessage(0);
    	return 0;
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    	if((HWND) lParam == hActiveB && index == 0){
    		...
    		idx02.lpParameters = "/c slmgr /skms kms.ddns.net";
    		...
    		if(GetLastError() != 0){
    			...
    			MessageBox(hwnd, "An error occurred while activating Windows", "Activate Windows 10/11", MB_ICONERROR|MB_APPLMODAL);
    			EnableWindow(hActiveB, TRUE);
    			EnableWindow(hActiveVerWin, TRUE);
			}
			else{
				FlashWindow(hwnd, TRUE);
    			msgboxid = MessageBox(hwnd, "Do you want to open activation settings?", "Activate Windows 10/11", MB_ICONINFORMATION|MB_APPLMODAL|MB_YESNO);
			}
		}
		if((HWND) lParam == hActiveVerWin && index == 0){
   			SetWindowText(hActiveStringKey2, "NW6C2-QMPVW-D7KKK-3GKT6-VCFB2");
   			SetWindowText(hActiveStringKMS2, "kms.ddns.net");
   			EnableWindow(hActiveB, TRUE);
   		}
    	if((HWND) lParam == hActiveB && index == 1){
    		...
    		idx12.lpParameters = "/c slmgr /skms kms.ddns.net";
    	...
    	if((HWND) lParam == hActiveB && index == 2){
    		...
    		idx22.lpParameters = "/c slmgr /skms kms.ddns.net";
    	...
    	if((HWND) lParam == hActiveB && index == 3){
    		...
    		idx32.lpParameters = "/c slmgr /skms kms.ddns.net";
    	...
    	if((HWND) lParam == hActiveB && index == 4){
    		...
    		idx42.lpParameters = "/c slmgr /skms kms.ddns.net";
    	...
    	if((HWND) lParam == hActiveB && index == 5){
    		...
    		idx52.lpParameters = "/c slmgr /skms kms.ddns.net";
		...
		if((HWND) lParam == hActiveVerWin && index == 5){
   		...
   		if((HWND) lParam == hActiveB && index == 6){
    		...
    		idx62.lpParameters = "/c slmgr /skms kms.ddns.net";
		...
		if((HWND) lParam == hActiveVerWin && index == 6){
   		...
   		if((HWND) lParam == hActiveB && index == 7){
    		...
    		idx72.lpParameters = "/c slmgr /skms kms.ddns.net";
		...
		if((HWND) lParam == hActiveVerWin && index == 7){
   		...
   		if((HWND) lParam == hActiveB && index == 8){
    		...
    		idx82.lpParameters = "/c slmgr /skms kms.ddns.net";
		...
		if((HWND) lParam == hActiveVerWin && index == 8){
   		...
   		if((HWND) lParam == hActiveB && index == 9){
    		...
    		idx92.lpParameters = "/c slmgr /skms kms.ddns.net";
		...
		if((HWND) lParam == hActiveVerWin && index == 9){
   		...
   		if((HWND) lParam == hActiveBCheck){
   			ShellExecute(NULL, "open", "C:\\Windows\\system32\\cmd.exe", "/c slmgr /dli && slmgr /xpr", NULL, SW_HIDE);
		   }
		if((HWND) lParam == hActiveAbout){
			MessageBox(hwnd, " Activate Windows 10/11 by bako35 \n Ver. 1.0.1 \n github.com/bako35", "About", MB_ICONINFORMATION|MB_APPLMODAL);
		}
